...
import util.S_ExpressionHandler;
...
public class GpTreeManager {
	...
	public static List<GpNode> rampedHalfAndHalf(GpEnvironment<?> environment) {
		...
		for (int i = 2; i <= stepSize; i++) {
			for (; index < i * (environment.getPopulationSize() / stepSize)
					- (environment.getPopulationSize() / stepSize) / 2; index++) {
	...
	/**
	 * crossover operation. it returns two children as results of subtree
	 * exchanging. This is the implementation of uniform crossover.
	 * 
	 * @param environment
	 * @return two children
	 */
	...
	/**
	 * crossover operation. it returns two children as results of subtree
	 * exchanging. Depth Dependent crossover operator.
	 * 
	 * @param environment
	 * @return two children
	 */
	public static GpNode[] crossoverDepthDependent(GpNode treeA, GpNode treeB,
		...
		while (crossoverNodeB.getDepthFromHere() + crossoverNodeA.getDepth() - 1 > environment.getNumberOfMaxDepth()
				|| crossoverNodeA.getDepthFromHere() + crossoverNodeB.getDepth() - 1 > environment
						.getNumberOfMaxDepth()) {
	 ...
	 * @return two children
	 */
	public static GpNode[] crossover(GpNode treeA, GpNode treeB, GpNode randomNodeA, GpNode randomNodeB,
			GpEnvironment<? extends GpIndividual> environment) {
		...
		if (randomNodeA == treeA) {
			...
			randomNodeA.getParent().getChildren()
					.set(randomNodeA.getParent().getChildren().indexOf(randomNodeA), randomNodeB);
		}
		if (randomNodeB == treeB) {
			treeB = randomNodeA;
		} else {
			randomNodeB.getParent().getChildren()
					.set(randomNodeB.getParent().getChildren().indexOf(randomNodeB), randomNodeA);
		}
	...
	public static GpNode mutation(GpNode tree, GpEnvironment<? extends GpIndividual> environment) {
		...
		if (randomNode == child) // if root node is selected
		{
			...
			randomNode.getParent().getChildren()
					.set(randomNode.getParent().getChildren().indexOf(randomNode), randomTree);
		}
	...
	/**
	 * retrieves tree and returns the size of tree. BFS order. Restriction: same
	 * depth
	 * 
	 * @param node
	 * @return size of the tree
	...
	/**
	 * retrieves tree and returns a node at specified index. Restriction: same
	 * depth
	 * 
	 * @param node
	 * @return node at specified index
	...
	public static GpNode constructGpNodeFromString(String str, GpSymbolSet set) {
		...
		for (int i = 0; i < rootSymbolNode.getChildCount(); i++) {
			...
			GpNode node = new GpNode(set.getSymbolByName((String) ((DefaultMutableTreeNode) rootSymbolNode
					.getChildAt(i)).getUserObject()), 1);
			extraValue = set.getExtraValueByName((String) ((DefaultMutableTreeNode) rootSymbolNode.getChildAt(i))
					.getUserObject());
			if (extraValue != null) {
	...
	/**
	 * returns tree string for S expression form. It returns "" if specified
	 * node is null.
	 * 
	 * @param node
	 * @return
	...
	public static void main(String args[]) {
		...
		System.out.println("Child A = " + getS_Expression(children[0]));
		System.out.println("Child B = " + getS_Expression(children[1]));
